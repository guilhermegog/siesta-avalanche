...
def get_indices(ix_dir, min_class, max_class, training, dataset_name):
    train_labels = np.load(os.path.join(
        ix_dir, '{}_indices/{}_train_labels.npy'.format(dataset_name, dataset_name)))
    val_labels = np.load(os.path.join(
        ix_dir, '{}_indices/{}_val_labels.npy'.format(dataset_name, dataset_name)))
    if training:
        curr_idx = get_imagenet_indices(
            train_labels, min_val=min_class, max_val=max_class)
        curr_labels = train_labels[np.array(curr_idx)]
    else:
        curr_idx = get_imagenet_indices(
            val_labels, min_val=min_class, max_val=max_class)
...
def filter_by_class(labels, min_class, max_class):
    ...
    """
    ixs = list(np.where(np.logical_and(
        labels >= min_class, labels < max_class))[0])
    return ixs


def get_imagenet_dataset(dirname, label_dir, split, batch_size=128, shuffle=False, min_class=0, max_class=None,
    ...
    if max_class is not None:
        _labels = np.load(
            os.path.join(label_dir, '{}_files/{}_{}_labels.npy'.format(dataset_name, dataset_name, split)))
        idxs = filter_by_class(
            _labels, min_class=min_class, max_class=max_class)
