...
class Accuracy(Metric[float]):
    """Top-5 Accuracy metric. This is a standalone metric.

    The update method computes the top-5 accuracy incrementally
    by keeping a running average of the <prediction, target> pairs
    of Tensors provided over time.

    The "prediction" tensor should contain logits or probabilities, while
    "target" tensor may contain labels or one-hot vectors.

    Each time `result` is called, this metric emits the average top-5 accuracy
    ...
    def __init__(self):
        """Creates an instance of the standalone Top-5 Accuracy metric.
    ...
    def update(
        ...
        """Update the running top-5 accuracy given the true and predicted values.

        :param predicted_y: The model prediction. Must be logits or probability vectors.
        ...
        # Convert to labels if given as one-hot vectors
        ...
        if len(predicted_y.shape) == 1:
            raise ValueError(
                "Top-5 accuracy requires logits or probability vectors as predictions")

        # Get the top 5 predictions (or less if fewer classes)
        k = min(5, predicted_y.shape[1])
        _, predicted_top5 = torch.topk(predicted_y, k=k, dim=1)

        # Expand true labels for comparison
        true_y_expanded = true_y.unsqueeze(1).expand_as(predicted_top5)

        # Check if true label is in top k predictions
        correct_top5 = torch.eq(predicted_top5, true_y_expanded).any(dim=1)
        accuracy = float(torch.sum(correct_top5)) / len(true_y)

        self._mean_accuracy.update(accuracy, len(true_y))

    def result(self) -> float:
        """Retrieves the running top-5 accuracy.

        Calling this method will not change the internal state of the metric.

        :return: The current running top-5 accuracy, which is a float value
            between 0 and 1.
        """
...
class AccuracyPerTaskPluginMetric(
    ...
    def update(self, strategy):
        self._metric.update(strategy.mb_output,
                            strategy.mb_y, strategy.mb_task_id)
